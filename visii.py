...
class StableDiffusionVisii(DiffusionPipeline):
    r"""
    ...
    def _encode_prompt(
        ...
        if prompt_embeds is None:
            ...
            eos_location = torch.where(text_input_ids[0] == 49407)[0].numpy()[0]
        ...
        if return_eos:
            return prompt_embeds, eos_location
        else:
    ...
    def text_projection(self, cond_image, target_image, device='cuda'):
        with torch.no_grad():
            ...
            img_inputs = processor(images=[cond_image, target_image], return_tensors="pt").to(device)
            
            pooled_features = clip_model.vision_model(img_inputs['pixel_values'])[1]
            pooled_features = clip_model.visual_projection(pooled_features)
            before_embed = pooled_features[0:1]
            after_embed = pooled_features[1:2]

            edit_direction_embed = after_embed - before_embed
    ...
    def text_projection2(self, cond_images, target_images, device='cuda'):
        with torch.no_grad():
            clip_model = CLIPModel.from_pretrained("openai/clip-vit-large-patch14")
            processor = AutoProcessor.from_pretrained("openai/clip-vit-large-patch14")
            clip_model.eval().to(device)
    ...
    def train(
        ...
        target_images: Union[torch.FloatTensor, PIL.Image.Image],
        cond_images: Union[torch.FloatTensor, PIL.Image.Image],
        ...
        callback_steps: int = 1,
        exp_name: str = 'test',
        lambda_clip: Optional[float] = 0.1,
        lambda_mse: Optional[float] = 4,
        eval_step: int=200,
        clip_loss: bool = True,
        log_dir: str = './logs',
        top_percent: int = 10,
        **kwargs,
        ...
        # cond_image_orig = cond_image.copy()
        ...
        if prompt_embeds is None:
            prompt_embeds, eos_location = self._encode_prompt(
                prompt,
                device,
                num_images_per_prompt,
                do_classifier_free_guidance,
        ...
        prompt_embeds_orig = prompt_embeds.clone()

        optimizer = bnb.optim.AdamW8bit(
            [prompt_embeds],  # only optimize the embeddings
        ...
        for i in range(0, 1000):
            noises.append(torch.randn(target_image_latents.shape).to(target_image_latents.device))
        torch.save(torch.cat(noises), os.path.join(log_dir, exp_name, 'noises.pt'))

        total_step = 0
        universal_mse = 100
        # for num_img in range(len(list_target_image_latents)):
        total_index = [x for x in range(0, 1000)]
        # breakpoint()
        num_img = len(cond_list)
        total_index = np.concatenate([total_index]) # TODO: * times number of iterations
    ...
    def test_fixed_noise(
        ...
        prompt: Union[str, List[str]] = None,
        image: Union[torch.FloatTensor, PIL.Image.Image] = None,
        noises: Optional[torch.FloatTensor] = None,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # compute the previous noisy sample x_t -> x_t-1
                latents = self.scheduler.step2(noise_pred, t, latents, noise=noises[i:(i+1)], **extra_step_kwargs).prev_sample
    ...
    def test_concatenate(
        ...
        callback: Optional[Callable[[int, int, torch.FloatTensor], None]] = None,
        callback_steps: int = 1,
        init_prompt: Optional[Union[str, List[str]]] = None,
    ):
        # 0. Check inputs
        # self.check_inputs(prompt, callback_steps)
        if image is None:
        ...
        scheduler_is_in_sigma_space = hasattr(self.scheduler, "sigmas")
        # 2. Encode input prompt
        prompt_embeds_ins, eos_location_ins = self._encode_prompt(
            init_prompt,
            device,
            num_images_per_prompt,
            do_classifier_free_guidance,
        ...
        # breakpoint()
        prompt_embeds_2, eos_location_2 = self._encode_prompt(
            prompt,
            device,
            num_images_per_prompt,
            do_classifier_free_guidance,
        ...
        # offset overlapping location = 2
        overlaping_start = eos_location_ins+2
        prompt_embeds[0, overlaping_start:(overlaping_start+eos_location_2)] = prompt_embeds_2[0, 0:eos_location_2]
